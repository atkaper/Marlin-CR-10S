...
void lcd_quick_feedback(const bool clear_buttons) {
  ...
  void lcd_cooldown() {
    ...
    void _lcd_goto_next_corner() {
      ...
      switch (bed_corner) {
        ...
        #else
          case 4:
            // no break, fall through to 5
            bed_corner = 5;
        #endif
        // TKA added parking position
        case 5:
          current_position[X_AXIS] = X_MIN_BED + LEVEL_CORNERS_INSET;
          current_position[Y_AXIS] = Y_MIN_BED + LEVEL_CORNERS_INSET;
          break;
      ...
      // TKA move up
      line_to_z(bed_corner < 5 ? 0.0 : 20.0);
      if (++bed_corner > 5) bed_corner = 0;
    }
    ...
    // TKA 2/1/2019 renamed (added _homed)
    void _lcd_level_bed_corners_homed() {
      defer_return_to_status = true;
    ...
    // TKA 2/1/2019 added auto home (if needed) to corner leveling...
    void _lcd_level_bed_corners() {
      if (!all_axes_homed()) {
        defer_return_to_status = true;
        axis_homed = 0;
        lcd_goto_screen(_lcd_level_bed_corners_homed);
        enqueue_and_echo_commands_P(PSTR("G28"));
      } else {
        _lcd_level_bed_corners_homed();
      }
    }
    ...
    void lcd_bed_leveling() {
      ...
      //
        ...
        // TKA updated to run unconditional: if (all_axes_homed()) MENU_ITEM(submenu, MSG_LEVEL_CORNERS, _lcd_level_bed_corners);
  ...
  void lcd_prepare_menu() {
    ...
    //
      ...
      // // TKA updated to run unconditional: if (all_axes_homed())
